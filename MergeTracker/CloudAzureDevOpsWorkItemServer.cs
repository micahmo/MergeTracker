...
{
    public class CloudAzureDevOpsWorkItemServer : IWorkItemServer
    {
        ...
        {
            if (_client != null && _networkCredential.Password == RootConfiguration.Instance.CloudAzureDevOpsToken)
            {
                return _client;
            }

            VssCredentials vssCredentials = new VssBasicCredential(new VssBasicToken(_networkCredential = new NetworkCredential(string.Empty, RootConfiguration.Instance.CloudAzureDevOpsToken)));
